      ...
      if (playPromise !== undefined) {
        playPromise.catch(error => {
          ...
          setTimeout(() => {
            videoElement
              .play()
              .catch(e => console.log("Manual play also failed:", e))
          }, 100)
        })
      }
    }
                        ...
                        <video
                          ref={el => {
                            try {
                              ...
                              console.warn(
                                `Error setting video ref for publication ${publication.id}:`,
                                error,
                              )
                            }
                          }}
                          ...
                          onError={() => {
                            try {
                              ...
                              console.warn(
                                `Error in video error handler for publication ${publication.id}:`,
                                error,
                              )
                            }
                          }}
                          onLoadStart={() => {
                            try {
                              console.log(
                                `Loading video for publication ${publication.id}`,
                              )
                            } catch (error) {
                              console.warn(
                                `Error in video load start handler for publication ${publication.id}:`,
                                error,
                              )
                            }
                          }}
                          onCanPlay={e => {
                            try {
                              console.log(
                                `Video can play for publication ${publication.id}`,
                              )
                              forceVideoPlay(e.target)
                            } catch (error) {
                              console.warn(
                                `Error in video can play handler for publication ${publication.id}:`,
                                error,
                              )
                            }
                          }}
                          onLoadedData={e => {
                            try {
                              console.log(
                                `Video loaded for publication ${publication.id}`,
                              )
                              forceVideoPlay(e.target)
                            } catch (error) {
                              console.warn(
                                `Error in video loaded data handler for publication ${publication.id}:`,
                                error,
                              )
                            }
                          }}
                          onEnded={e => {
                            try {
                              console.log(
                                `Video ended for publication ${publication.id}, restarting loop`,
                              )
                              handleVideoEnded(e.target)
                            } catch (error) {
                              console.warn(
                                `Error in video ended handler for publication ${publication.id}:`,
                                error,
                              )
                            }
                          }}
